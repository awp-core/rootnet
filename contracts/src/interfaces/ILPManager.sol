// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;

/// @title ILPManager — Liquidity management interface
/// @notice Wraps PancakeSwap V4 / Uniswap V4 LP creation; LP is permanently locked
interface ILPManager {
    /// @notice Create an LP pool and add full-range liquidity (called once during worknet activation)
    /// @param worknetToken Worknet token address
    /// @param awpAmount AWP amount
    /// @param worknetTokenAmount Worknet token amount
    /// @return poolId LP pool ID (bytes32)
    /// @return lpTokenId LP NFT ID (LP permanently locked in LPManager)
    function createPoolAndAddLiquidity(address worknetToken, uint256 awpAmount, uint256 worknetTokenAmount)
        external
        returns (bytes32 poolId, uint256 lpTokenId);
}
