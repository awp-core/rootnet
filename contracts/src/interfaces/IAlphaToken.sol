...
/// @title IAlphaToken — Worknet Alpha token interface
/// @notice Clonable upgrade pattern; one instance per worknet; MAX_SUPPLY = 10B
interface IAlphaToken is IERC20 {
    ...
    function initialize(string memory name, string memory symbol, uint256 worknetId, address admin) external;
    /// @notice Mint (authorized minters only, not in paused state)
    function mint(address to, uint256 amount) external;
    /// @notice Set worknet contract as sole minter and permanently lock
    function setWorknetMinter(address worknetManager) external;
    /// @notice Pause/resume a specific minter (used for worknet banning)
