...
/// @notice Deploys AlphaToken instances for each worknet via CREATE2
interface IAlphaTokenFactory {
    ...
    /// @param salt CREATE2 salt; bytes32(0) skips vanity validation and uses worknetId as salt
    function deploy(uint256 worknetId, string memory name, string memory symbol, address admin, bytes32 salt)
        external
        returns (address);
}
