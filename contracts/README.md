...
|----------|-------------|
| `AWPRegistry` | Unified entry: worknet lifecycle + staking allocation. Epoch authority (immutable genesisTime + epochDuration). |
...
| `AlphaToken` | Per-worknet ERC20 via CREATE2. Time-based mint cap. |
...
| `StakingVault` | Pure allocation logic. (user, agent, subnetId) triples. Auto-enumerates agent worknets. |
| `SubnetNFT` | ERC721. tokenId = subnetId. Ownership = worknet control. |
| `SubnetManager` | Default worknet contract (PancakeSwap V4). Merkle claim, AWP strategy, deployed behind ERC1967Proxy. |
